...
const parseData = (data: any) => {
    ...
    return {
        products: (data.products || []).map((p: any) => ({...p, isAvailable: p.isAvailable ?? true})),
        categories: data.categories || [],
        customers: data.customers || [],
        expenses: parseWithDates(data.expenses, 'date'),
        orders: parseWithDates(data.orders, 'createdAt'),
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const updateSettings = (newSettings: Partial<AppSettings>) => {
    ...
    delete settingsToUpdate.currency;
  ...
  const addOrder = async (customerData: Omit<Customer, 'id'>, items: CartItem[]): Promise<boolean> => {
    ...
    if(result.status === 'success') {
        ...
        // Add new order
        setOrders(prev => [{...newOrder, createdAt: new Date(newOrder.createdAt)}, ...prev].sort((a,b) => b.createdAt.getTime() - a.createdAt.getTime()));
