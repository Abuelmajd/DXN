        ...
        <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              <NavLink to="/admin" className="text-white text-xl font-bold flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                </svg>
                <span>{t('salesManager')}</span>
              </NavLink>
              <div className="ms-4 border-s border-indigo-400 ps-4 hidden md:block">
            ...
            <div className="hidden md:flex items-center space-s-4">
              ...
              <NavLink to="/admin/new-order" className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-emerald-500 hover:bg-emerald-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                {t('newInvoice')}
              </NavLink>
              <NavLink to="/admin/setup-check" title={t('setupCheck')} className="text-slate-300 hover:text-white transition-colors p-2 rounded-full hover:bg-indigo-500">
                <ChecklistIcon />
              </NavLink>
              <NavLink to="/admin/settings" title={t('settings')} className="text-slate-300 hover:text-white transition-colors p-2 rounded-full hover:bg-indigo-500">
                <SettingsIcon />
              </NavLink>
              <button onClick={handleLogout} title={t('logout')} className="text-slate-300 hover:text-white transition-colors p-2 rounded-full hover:bg-indigo-500">
                <LogoutIcon />
              </button>
            </div>
            ...
            <div className="md:hidden">
                <button onClick={() => setIsMenuOpen(true)} className="text-slate-200 hover:text-white p-2 rounded-md">
        ...
        <div className={`absolute top-0 bottom-0 w-64 bg-slate-50 dark:bg-slate-800 shadow-xl transition-transform duration-300 ease-in-out ${isRTL ? 'right-0' : 'left-0'} ${isMenuOpen ? 'translate-x-0' : (isRTL ? 'translate-x-full' : '-translate-x-full')}`}>
            ...
            <div className="h-[calc(100vh-65px)] overflow-y-auto">
                ...
                <div className="p-4 space-y-2">
                    <NavLink to="/admin/new-order" onClick={() => setIsMenuOpen(false)} className="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-emerald-500 hover:bg-emerald-600">
                        {t('newInvoice')}
                    </NavLink>
                </div>
