...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden flex flex-col transition-transform duration-300 ease-in-out hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 flex-grow text-right">{product.description || t('noDescriptionAvailable')}</p>
        <div className="mt-4">
          ...
          <div className="flex justify-between items-center mt-1">
            <span className="text-md font-bold text-indigo-600 dark:text-indigo-400">{t('memberPrice')}</span>
            <span className="text-xl font-bold text-indigo-600 dark:text-indigo-400">{formatCurrency(product.memberPrice)}</span>
          </div>
        </div>
      </div>
    </div>
