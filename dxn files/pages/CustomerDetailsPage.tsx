...
const CustomerDetailsPage: React.FC = () => {
    ...
    if (!customer) {
        return (
             <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-xl shadow-lg">
                ...
                <Link to="/admin/customers" className="mt-4 inline-block text-indigo-600 dark:text-indigo-400 hover:underline">
                    {t('backToCustomers')}
                </Link>
            </div>
        )
    ...
    return (
        <div className="max-w-4xl mx-auto">
             <div className="mb-6">
                <Link to="/admin/customers" className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-2 mb-4">
             ...
             <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 mb-8 space-y-3">
                {customer.phone && (
                    <a href={`tel:${customer.phone}`} className="flex items-center gap-3 text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors font-medium">
                        <PhoneIcon />
                ...
                {customer.whatsapp && (
                 ...
                 {customer.email && (
                    <a href={`mailto:${customer.email}`} className="flex items-center gap-3 text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors font-medium">
                        <EmailIcon />
                            ...
                            <details className="p-4 group" open={customerOrders.length === 1}>
                                <summary className="flex justify-between items-center cursor-pointer list-none">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <span className="font-bold text-lg text-emerald-600 dark:text-emerald-400">{formatCurrency(order.totalPrice)}</span>
